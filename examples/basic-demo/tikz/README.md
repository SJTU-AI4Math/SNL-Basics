...
- `generated/higher-category.template.svg` is derived from that file. The generator reads each formula's exact `dvisvgm` local bounding box, removes its glyph group, and inserts its ordered empty `data-snl-slot` occurrence at the same center.
