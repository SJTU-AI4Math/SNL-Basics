...
describe('basic demo Entry integration route', () => {
  it('renders and previews through the public Entry path rather than the bare syntax-tree view', () => {
    ...
    expect(app).toContain('defaultRenderers')
    expect(presets).toContain("block_template_name: 'svg_template'")
    expect(presets).toContain('formula_embed')
    expect(app).toContain("renderers: { ...defaultRenderers, svg_template: svgRenderer }")
    expect(app).toContain("'Layout.right':")
    expect(app).toContain("block_template_name: 'right'")
    expect(app).toContain("Layout.right(Left,Right)")
