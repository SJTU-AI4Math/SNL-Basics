...
export default function App() {
  ...
  const [templateDb] = useState<SnlMacroRecord>({})
  ...
  return (
    <div className="page" onKeyDownCapture={handleKeyDownCapture}>
      ...
      <div className="section">
        <h2>1) Parser 输入</h2>
        <p>
          语法：name 后可跟可选的 [style] 方括号覆盖渲染样式，再跟
          (child1,child2(…))。例如 群.示例(@x,x) 同时演示 Unicode 名称和显式 binder。
          宏数据由消费者通过 MacroDataDriver 提供；Basics 不附带 Macro DB。
        </p>
        <div className="row">
          <input
            value={expression}
